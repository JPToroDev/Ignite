...
extension StyleManager {
    ...
    ///   - selector: The selector to use in the declaration block. Uses
    ///    the autogenerated class name of the `Style` when `nil`
    ///   - isImportant: Whether the styles should be marked `!important`
    ...
    func generateCSS(
        ...
        for (condition, styles) in stylesMap.uniqueConditions {
            ...
            let styles = styles.map { $0.important(isImportant) }
